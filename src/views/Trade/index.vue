...
import { mapGetters, mapState } from 'vuex'
  export default {
    ...
    computed:{
      ...mapState({addressInfo:state => state.trade.addressInfo}),
    ...
    methods:{
      changeDefault(index){
        ...
        this.addressInfo[index].isDefault = 1

      },
      async submitOrder(){
        let {tradeNo} = this.tradeInfo
        let {consignee} = this.userDefaultAddress
        let deliveryAddress = this.userDefaultAddress.fullAddress
        let consigneeTel = this.userDefaultAddress.phoneNum
        let paymentWay = "ONLINE"
        let orderComment = this.message
        let orderDetailList = this.detailArrayList
        const data = {
          tradeNo,
          consignee,
          deliveryAddress,
